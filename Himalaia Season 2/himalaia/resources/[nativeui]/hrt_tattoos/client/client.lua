...
vSERVER = Tunnel.getInterface("gvt_tattoos")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIABLES
-----------------------------------------------------------------------------------------------------------------------------------------
local cam = nil
...
-- SETTATTOS
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("gvt_tattoos:setTattoos")
AddEventHandler("gvt_tattoos:setTattoos",function(status)
	atualTattoo = status
	...
	while true do
		...
		if not IsPedInAnyVehicle(ped) then
			...
			for k,v in pairs(coordsSystem) do
				...
				if distance <= 10 then
					timeDistance = 4
					DrawMarker(23,v[1],v[2],v[3]-0.95,1,1,0,0,0,0,0.4,0.4,0.4,255,255,255,50,0,1,0,0)
...
Citizen.CreateThread(function()
	while true do
		...
		projectC = 5
		if GetDistanceBetweenCoords(GetEntityCoords(ped), -558.95739746094,-216.53158569336,37.649787902832,true) <= 1 then
			TextoMarker(-558.95739746094,-216.53158569336,37.649787902832+1.0, "PRESSIONE ~g~E~s~ PARA ACESSAR A LOJA DE ~g~TATUAGENS.", 255)
		end
		Citizen.Wait(projectC)
	end
end)
