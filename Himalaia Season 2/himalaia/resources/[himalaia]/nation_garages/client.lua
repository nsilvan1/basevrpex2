...
--- TABLE QUE ARMAZENA AS GARAGENS PRÓXIMAS -----https://discord.gg/sergin
nearestGarages = {}
...
--- CLASSES DOS VEÍCULOS PARA ENVIAR À NUI -----https://discord.gg/sergin
local classes = {
    [0] = "Compacts",
    [1] = "Sedans",
    [2] = "SUVs",
...
--- RETORNA A CLASSE DE UM VEÍCULO PELO ÍNDICE -----https://discord.gg/sergin
function getVehicleClass(class) return classes[class] or "none" end

function cooldown() end

--- TUNNEL DO TOGGLE DA NUI -----https://discord.gg/sergin
function fclient.toggleNui() if func.checkAuth() then toggleNui() end end

--- TOGGLE DA NUI -----https://discord.gg/sergin
...
--- ABRIR GARAGEM PÚBLICA -----https://discord.gg/sergin
function openPublicGarage()
    if func.checkAuth() then
        ...
        SendNUIMessage({
            action = "show",
            vehicles = vehicles,
            seguradora = config.seguradora / 100,
            ipva = ((config.ipva or 1) / 100),
            detido = config.detido / 100,
            defaultImg = config.defaultImg,
            imgDir = config.imgDir,
            saveNearestVeh = config.guardar_veiculo_proximo,
            time = func.getTime()
        })
        nui = true
    end
...
--- ABRIR GARAGEM DE SERVIÇO -----https://discord.gg/sergin
...
--- FECHAR A NUI -----https://discord.gg/sergin
...
--- RETORNA UMA TABLE COM TODOS OS VEÍCULOS DO PLAYER E SUAS INFORMAÇÕES -----https://discord.gg/sergin
...
--- RETORNA OS VEÍCULOS DE UMA DETERMINADA GARAGEM DE SERVIÇO -----https://discord.gg/sergin
function getServiceVehicles(garage)
    if func.checkAuth() then
        ...
        for i in ipairs(vehicles) do
            ...
            vehicles[i].engine, vehicles[i].body, vehicles[i].fuel = 1000, 1000,
                                                                     100
        end
        return vehicles or {}
    end
...
--- THREAD QUE ATUALIZA AS GARAGENS PRÓXIMAS -----https://discord.gg/sergin
...
--- THREAD PARA DESENHAR OS MARKERS (GARAGENS PRÓXIMAS) E ABRIR A NUI CASO O PLAYER PRESSIONE "E" -----https://discord.gg/sergin
Citizen.CreateThread(function()
    while true do
        local idle = 500
...
--- CHECA SE O VEÍCULO JÁ ESTÁ NA RUA -----https://discord.gg/sergin
...
--- CHECA SE TEM VAGA PARA SPAWNAR O VEÍCULO -----https://discord.gg/sergin

function fclient.checkSpot(vehicleInfo)
    if func.checkAuth() then
        local modelName =
            GetDisplayNameFromVehicleModel(vehicleInfo.vehicle):lower()
        if checkInStreet(modelName) then
            closeGarage()
            TriggerEvent("Notify", "negado",
                         "Você já tem um veículo deste modelo fora da garagem.",
                         3000)
            return
        end
        local count = 0
        for _, spawn in ipairs(nearestGarage.vehiclePositions) do
            ...
            local closestVehicleOnSpot =
                GetClosestVehicle(spawnCoords.x, spawnCoords.y, spawnCoords.z,
                                  3.001, 0, 71)
            if DoesEntityExist(closestVehicleOnSpot) then
                ...
                if count >= #nearestGarage.vehiclePositions then
                    toggleNui()
                    TriggerEvent('Notify', 'negado',
                                 'Todas as vagas estão ocupadas no momento.',
                                 3000)
                    return
                end
            else
                ...
                if spawnVehicle(vehicleInfo, spawnCoords, spawn.h) then
                    notify(vehicleInfo.vehicle, "retirou",
                           config.getVehicleModel(vehicleInfo.vehicle))
                end
                return
            end
        end
...
--- FUNÇÃO DE SPAWN DO VEÍCULO -----https://discord.gg/sergin

function spawnVehicle(vehicleInfo, coords, h)
    if func.checkAuth() then
		local mhash = GetHashKey(vehicleInfo.vehicle)
        if not mhash then
		...
		if HasModelLoaded(mhash) then
			...
			local nveh = CreateVehicle(mhash,coords.x,coords.y,coords.z,h,true,true)
			local nearestVeh = GetClosestVehicle(coords.x,coords.y,coords.z,3.001,0,71)
			IsVehicleModel(nearestVeh, vehicleInfo.vehicle)
			vehicle = nearestVeh
			...
			func.registerVehicle(VehToNet(vehicle),mhash,plate)
			return true
		end
		return false
    end
...
--- CHECA SE O VEÍCULO ESTÁ ATIVO (ALGUM PLAYER DEVE ESTAR DENTRO DO VEÍCULO PARA RETORNAR TRUE) -----https://discord.gg/sergin
    ...
    Citizen.CreateThread(function()
        if NetworkDoesEntityExistWithNetworkId(index) then
            ...
            if DoesEntityExist(v) then
                ...
                local plate, engine, body, fuel = GetVehicleNumberPlateText(v),
                                                  parseInt(
...
-- CARREGAR O MODEL DO VEÍCULO ----https://discord.gg/sergin
...
-- PEGAR O TAMANHO DA TABLE (ÍNDICES NÃO ORDENADOS) ----https://discord.gg/sergin
function length(array)
    local len = 0
    for i in pairs(array) do if array[i] then len = len + 1 end end
    return len
end

---- NOTIFICAÇÃO DOS VEÍCULOS -----https://discord.gg/sergin
...
-------- IDENTIFICAR VEÍCULOS PRÓXIMOS -------------https://discord.gg/sergin
...
function getNearestVehicles(radius)
    ...
    for _, veh in pairs(vehs) do
        ...
        if distance <= radius then r[veh] = distance end
    end
    return r
end

-- IDENTIFICAR O VEÍCULO MAIS PRÓXIMO ----https://discord.gg/sergin
...
-- IDENTIFICAR O RETORNAR INFORMAÇÕES SOBRE O VEÍCULO MAIS PRÓXIMO ----https://discord.gg/sergin
function fclient.getNearestVehicleInfo()
    local veh = GetVehiclePedIsIn(PlayerPedId(), false)
    if veh == 0 then veh = getNearestVehicle(7) end
    if DoesEntityExist(veh) then
        local v = func.getVehicleInfo(GetEntityModel(veh))
        if v then
            return GetVehicleNumberPlateText(veh), v.name, parseInt(v.price),
                   v.banido, VehToNet(veh), veh
        end
    end
end
...
------ TRANCAR E DESTRANCAR O VEÍCULO -------https://discord.gg/sergin
...
--- KEYMAPPING DO COMANDO +TRYLOCKVEHICLE -----https://discord.gg/sergin
...
--- FUNCAO TRANCAR / DESTRANCAR O VEÍCULO -----https://discord.gg/sergin
...
function fclient.toggleAnchor(vnet)
    local vehicle = NetToVeh(vnet)
    if DoesEntityExist(vehicle) and IsEntityAVehicle(vehicle) and
...
-- RETORNA A CAPACIDADE E O ESPAÇO OCUPADO NO PORTA-MALAS DE UM DETERMINADO VEÍCULO ----https://discord.gg/sergin
...
-- FECHAR --
RegisterNUICallback("close", function(data) toggleNui() end)

-- RETORNA TRUE CASO O PLAYER PAGUE A TAXA COM SUCESSO ----https://discord.gg/sergin
RegisterNUICallback("pay-tax", function(data, cb)
    if data and data.vehicle then cb(func.tryPayTax(data.vehicle)) end
end)

-- TENTA SPAWNAR O VEÍCULO --
RegisterNUICallback("spawn-vehicle", function(data)
    if data and data.vehicle and not block then
        func.checkVehicle(data.vehicle, nearestGarage.type,
    ...
    if data and data.vehicle and not block then
        ...
        if inStreet then
            ...
            if DoesEntityExist(vehicle) then
                ...
                if distance < 30 then
                    local plate, engine, body, fuel =
                        GetVehicleNumberPlateText(vehicle),
                        parseInt(GetVehicleEngineHealth(vehicle)),
                        parseInt(GetVehicleBodyHealth(vehicle)),
                        parseInt(GetVehicleFuelLevel(vehicle))
                    func.saveVehicle(data.vehicle, plate, engine, body, fuel)
                    TriggerServerEvent("nation:deleteVehicleSync", netveh)
                    notify(data.vehicle, "guardou",
                           config.getVehicleModel(data.vehicle))
                    closeGarage()
                end
            end
        end
        ...
        if DoesEntityExist(vehicle) then
            ...
            if distance < 30 then
                ...
                if vehInfo and vehInfo.name then
                    func.saveVehicle(vehInfo.name, plate, engine, body, fuel)
                    TriggerServerEvent("nation:deleteVehicleSync",
                                       VehToNet(vehicle))
                    notify(vehInfo.name, "guardou",
                           config.getVehicleModel(vehInfo.name))
                end
                closeGarage()
            end
        end
