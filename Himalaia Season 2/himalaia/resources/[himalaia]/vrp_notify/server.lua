...
RegisterCommand('adm2',function(source,args,rawCommand)
    ...
    if vRP.hasPermission(user_id,"admin.permissao") or vRP.hasPermission(user_id,"chat.permissao") then
...
RegisterCommand('anuncio2',function(source,args,rawCommand)
    local user_id = vRP.getUserId(source)
    local identity = vRP.getUserIdentity(user_id)
	if vRP.hasPermission(user_id,"admin.permissao") or vRP.hasPermission(user_id,"helper.permissao") then
        local mensagem = vRP.prompt(source,"Mensagem:","")
        if mensagem == "" then
            return
        end
