...
$(document).ready(function() {
    document.onkeydown = function(data) {
        if (data.keyCode == 27) {
            ...
            $('#cores').slideUp(0);
            $('#totalDireita').html('0'); 
            $.post('http://nyo_barbershop/reset', JSON.stringify({}));
        }
    }
    ...
    window.addEventListener('message', function(event) {
        ...
        if (item.openBarberShop) {
            ...
            $.post('http://nyo_barbershop/updateGui', JSON.stringify({part: dataPart}));  
        }        
        ...
        if(item.updateBarberShop){
            ...
            for (var i = 0; i <= item.drawable; i++) {    
                ...
                // `);
                 $("#listagem").append(`
                    <div class="lista" data-id="${i}" onclick="select(this)" id="${dataPart}${i}" >
                        <div class="valorItem">R$500 </div>
                        <div class="idItem">${i}</div>
                    </div>
                `);
...
function changeColor(color, type){
    $.post('http://nyo_barbershop/changeColor', JSON.stringify({ dataType: dataPart, color: color, type: type }));
}
...
function select(element) {
    ...
    $.post('http://nyo_barbershop/changeCustom', JSON.stringify({ type: dataPart, id: partId }));    
}
...
$("#esquerda").click(function() {
    $.post('http://nyo_barbershop/leftHeading', JSON.stringify({ value: 10 }));
})

$("#direita").click(function() {
    $.post('http://nyo_barbershop/rightHeading', JSON.stringify({ value: 10 }));
})
...
$("#cart").click(function() {
    $("#container").fadeOut();
    $('#cores').slideUp(0);
    $.post('http://nyo_barbershop/payament', JSON.stringify({ price: $('#totalDireita').text(), parts: oldPart }));
    $('#totalDireita').html('0');
})