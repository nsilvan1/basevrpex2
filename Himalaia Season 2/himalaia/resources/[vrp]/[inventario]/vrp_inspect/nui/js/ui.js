...
$(document).ready(function() {
  ...
  document.onkeyup = function(data) {
    if (data.which == 27) {
      $.post("http://vrp_inspect/chestClose", JSON.stringify({}), function(datab) {});
    }
  };
});
...
const updateChest = () => {
  ...
  $.post("http://vrp_inspect/requestChest", JSON.stringify({}), data => {
    ...
    $('.trunk-workspace > .inventory-title').html(`
      Inventário Proximo <small>(<i><b>${data.peso2.toFixed(2)}kg</b> em uso - <b>${(data.maxpeso2 - data.peso2).toFixed(2)}kg </b> livre de <b>${data.maxpeso2.toFixed(2)}kg</b></i> no total)</small>
    `);
