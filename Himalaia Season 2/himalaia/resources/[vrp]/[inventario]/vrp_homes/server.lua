...
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookbaucasas = "https://discord.com/api/webhooks/853699257504104479/YKTMstGRNc0xMostT4dgYEIy8ouM95HRUq3bjh8ztFMFcoYfxKuY8K7I7R0A4Tc_GpUj"
	...
	if user_id then
		if not vRP.searchReturn(source,user_id) then
			...
			if myResult[1] or vRP.hasPermission(user_id,"policia.permissao") then
				if not beingUsed[homeName] then
					beingUsed[homeName] = true
					return true
				end
			end
		end
