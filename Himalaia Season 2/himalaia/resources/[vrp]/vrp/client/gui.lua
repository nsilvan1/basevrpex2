...
local anims = {
	...
	{ dict = "anim@heists@prison_heiststation@cop_reactions", anim = "cop_a_idle" },
	{ dict = "amb@world_human_cop_idles@female@base", anim = "base" },
...
RegisterKeyMapping("DntZ1accept","Aceitar chamado","keyboard","y")
RegisterKeyMapping("DntZ1reject","Rejeitar chamado","keyboard","u")
-----------------------------------------------------------------------------------------------------------------------------------------
-- ACCEPT
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand("DntZ1accept",function(source,args)
	local ped = PlayerPedId()
...
-- REJECT
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand("DntZ1reject",function(source,args)
	local ped = PlayerPedId()
	if not celular and GetEntityHealth(ped) > 101 then
...
RegisterKeyMapping("dntz1bindkb 1","Bind F1","keyboard","F1")
RegisterKeyMapping("dntz1bindkb 2","Bind F2","keyboard","F2")
RegisterKeyMapping("dntz1bindkb 3","Bind F3","keyboard","F3")
RegisterKeyMapping("dntz1bindkb left","Bind Left","keyboard","left")
RegisterKeyMapping("dntz1bindkb right","Bind Right","keyboard","right")
RegisterKeyMapping("dntz1bindkb up","Bind Up","keyboard","up")
RegisterKeyMapping("dntz1bindkb down","Bind Down","keyboard","down")

RegisterCommand("dntz1bindkb",function(source,args)
	if not IsPauseMenuActive() then
		...
		if not menu_celular and GetEntityHealth(ped) > 101 and not menu_state.opened then
			if args[1] == "1" then
				if IsEntityPlayingAnim(ped,"anim@amb@nightclub@peds@","rcmme_amanda1_stand_loop_cop",3) then
					StopAnimTask(ped,"anim@amb@nightclub@peds@","rcmme_amanda1_stand_loop_cop",2.0)
--					tvRP.stopActived()
				else
...
RegisterKeyMapping("DntZ1point","Apontar os dedos","keyboard","b")

RegisterCommand("DntZ1point",function(source,args)
	if not IsPauseMenuActive() then
		local ped = PlayerPedId()
		if not IsPedInAnyVehicle(ped) and not cancelando and not celular and GetEntityHealth(ped) > 101 then
			tvRP.loadAnimSet2("anim@mp_point")
