...
AddEventHandler("yaga_antcl", function(user_id, crds, identifier, reason)
    Display(user_id, crds, identifier, reason)
end)

function Display(user_id, crds, identifier, reason)
    ...
    Citizen.CreateThread(function()
        while displaying do
            ...
            if GetDistanceBetweenCoords(crds.x, crds.y, crds.z, pcoords.x, pcoords.y, pcoords.z, true) < 15.0 and show3DText then
                DrawText3DSecond(crds.x, crds.y, crds.z+0.15, "Player deu CL")
                DrawText3D(crds.x, crds.y, crds.z, "ID: "..user_id.." ("..identifier..")\nReason: "..reason)
            else
                Citizen.Wait(2000)
            end
        end
