...
AddEventHandler('notificacao',function(x,y,z,user_id)
	...
	--if distance <= 2100 then
		if not DoesBlipExist(blips[user_id]) then
			...
			TriggerEvent("NotifyPush",{ code = 71, title = "Disparos Aconteceram, dirija-se até o local.", x = x, y = y, z = z, badge = "Disparos de arma de fogo" })
		...
		if prisioneiro then
			...
			if distance2 <= 100 and reducaopenal then
				...
				if distance2 <= 1.2 then
					...
					if IsControlJustPressed(0,38) then
						reducaopenal = false
						TriggerServerEvent("diminuirpenaexitofof00kkm")
						vRP._DeletarObjeto()
					end
				end
			end
