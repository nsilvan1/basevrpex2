...
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookspawn = "https://discord.com/api/webhooks/864675610936082492/u7wL9Xh93uaUxg3LmWuZVoO0S8D7ZCjqFBbR7IQtODUBGYQRjK-Pa_lvIrpnWEKqmRBP"
local webhookvehs = "https://discord.com/api/webhooks/864675666628837416/CpRxeX6WSSRDt3gSK6nprwLo33Vo66Ko2c7pX7AgAchDsx2BJQCIh5CRiBcyoj4co_aP"
local webhookdesmanche = "https://discord.com/api/webhooks/881667059006717972/ZmeHhC3kIGT15dsl--Oc56zsKrTcKm2m3EUoSaeXZp0N_fdZmSkLJECHt9SSrTIgct-k"
...
local garages = {
	...
	[45] = { ['name'] = "PoliciaI", ['payment'] = true, ['perm'] = "policia.permissao" },
	...
	[73] = { ['name'] = "Garagem", ['payment'] = true, ['perdm'] = "livre" },
	[74] = { ['name'] = "Ballas", ['payment'] = true, ['perm'] = "livre" },
	[75] = { ['name'] = "Vagos", ['payment'] = true, ['perm'] = "livre" },
	[76] = { ['name'] = "Groove", ['payment'] = true, ['perm'] = "livre" },
	[77] = { ['name'] = "Bloods", ['payment'] = true, ['perm'] = "livre" },
	[78] = { ['name'] = "Crips", ['payment'] = true, ['perm'] = "livre" },
	[79] = { ['name'] = "Triade", ['payment'] = true, ['perm'] = "livre" },
	[80] = { ['name'] = "Bratva", ['payment'] = true, ['perm'] = "livre" },
	[82] = { ['name'] = "Bahamas", ['payment'] = true, ['perm'] = "livre" },
	[84] = { ['name'] = "LifeInvader", ['payment'] = true, ['perm'] = "livre" },
-----------------------------------------------------------------------------------------------------------------------------------------
---------------------------------------------------------FORTHILLS-----------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------------------------
	...
	if name then
		...
		if user_id then
			...
			if multas >= 10000 then
				TriggerClientEvent("Notify",source,"negado","Negado","Você tem multas pendentes.",10000)
				return true
			end
			if not vCLIENT.returnVehicle(source,name) then
					...
					if parseInt(os.time()) <= parseInt(vehicle[1].time+24*60*60) then
						...
						if ok then
							if vRP.tryFullPayment(user_id,parseInt(vRP.vehiclePrice(name)*0.5)) then
								...
								TriggerClientEvent("Notify",source,"sucesso","Sucesso","Veículo liberado.",10000)
							else
						...
						if parseInt(os.time()) <= parseInt(vehicle[1].ipva+24*15*60*60) then
							if garages[use].payment then
								if vRP.vehicleType(tostring(name)) == "exclusive" or vRP.vehicleType(tostring(name)) == "rental" then
									...
									TriggerClientEvent("Notify",source,"sucesso","Sucesso","Veículo <b>Exclusivo ou Alugado</b>, Não será cobrado a taxa de liberação.",10000)
								end
								if (vRP.getBankMoney(user_id)+vRP.getMoney(user_id)) >= parseInt(vRP.vehiclePrice(name)*0.005 and not vRP.vehicleType(tostring(name)) == "exclusive" or vRP.vehicleType(tostring(name)) == "rental") then
									...
									if spawnveh and vRP.tryFullPayment(user_id,parseInt(vRP.vehiclePrice(name)*0.005)) then
										vehlist[vehid] = { parseInt(user_id),name }
										TriggerEvent("setPlateEveryone",identity.registration)
										TriggerClientEvent("Notify",source,"financeiro","Financeiro","Você pagou <b>$"..vRP.format(parseInt(vRP.vehiclePrice(name)*0.005)).." dólares</b>, da taxa de liberação.",10000)
									end
								else
									TriggerClientEvent("Notify",source,"negado","Negado","Dinheiro insuficiente.",10000)
								end
							...
							if vRP.vehicleType(tostring(name)) == "exclusive" or vRP.vehicleType(tostring(name)) == "rental" then
								...
								if ok then
									if vRP.tryFullPayment(user_id,parseInt(vRP.vehiclePrice(name)*0.00)) then
										...
										TriggerClientEvent("Notify",source,"sucesso","Sucesso","Pagamento do <b>Vehicle Tax</b> conclúido com sucesso.",10000)
									else
										TriggerClientEvent("Notify",source,"negado","Negado","Dinheiro insuficiente.",10000)
									end
								end
				...
				TriggerClientEvent("Notify",source,"aviso","Aviso","Já possui um veículo deste modelo fora da garagem.",10000)
			end
		end
	end
end
