...
AddEventHandler("vRPclient:playerSpawned",function(user_id,source) 
    ...
    TriggerClientEvent('banking:updateBalance9', source, bankbalance, multasbalance, walletbalance)
end)
...
function Depositar(user_id, amount)
  ...
  TriggerClientEvent("banking:updateBalance9", source, new_balance)
...
RegisterServerEvent('bank:update9')
AddEventHandler('bank:update9', function()
  local source = source
  local user_id = vRP.getUserId(source)
  local bankbalance = vRP.getBankMoney(user_id)
  ...
  TriggerClientEvent("banking:updateBalance9", source, bankbalance, walletbalance, multasbalance, identity.name.." "..identity.firstname) -- ADICIONAR VALORES AQUI PARA APARECER NA NUI + JAVA SCRIPT + HTML
end)

RegisterServerEvent('bank:pagarmulta9')
AddEventHandler('bank:pagarmulta9', function(amount)
  ...
  if vRP.tryFullPayment(user_id,rounded) then
    ...
    TriggerClientEvent("banking:updateBalance9", source, bank, wallet,novamulta)
    TriggerClientEvent("banking:removeMulta9", source, rounded)
...
RegisterServerEvent('bank:deposit9')
    ...
    if amount and type(amount) == "number" then
      ...
      if (rounded > 0) then
        ...
        if (rounded <= wallet) then
          ...
          TriggerClientEvent("banking:updateBalance9", source, new_balance,carteira)
          TriggerClientEvent("banking:addBalance9", source, rounded)
...
RegisterServerEvent('bank:withdraw9')
  ...
  if user_id then
    if amount and type(amount) == "number" then
      ...
      if (rounded <= bankbalance) then
        ...
        -- Update NUI
        TriggerClientEvent("banking:updateBalance9", source, new_balance, carteira)
        TriggerClientEvent("banking:removeBalance9", source, rounded)
        -- Salva o extrato
...
RegisterServerEvent('bank:quickCash9')
AddEventHandler('bank:quickCash9', function()
  local source = source
  local user_id = vRP.getUserId(source)
  ...
  if (bankbalance >= quantia) then
    ...
    local carteira = vRP.getMoney(user_id)
    TriggerClientEvent("banking:updateBalance9", source, new_balance,carteira)
    TriggerClientEvent("banking:removeBalance9", source, quantia)
...
RegisterServerEvent('bank:transfer9')
AddEventHandler('bank:transfer9', function(toPlayer, amount)
  local source = source
  local user_id = vRP.getUserId(source)
      ...
      if (rounded > 0) then
        ...
        if (rounded <= bankbalance) then
          ...
          if player then -- Está online
            ...
            TriggerClientEvent("banking:updateBalance9", player, newBalance_Player)
            TriggerClientEvent("banking:addBalance9", player, rounded)
          else
          ...
          local carteira = vRP.getMoney(user_id)
          TriggerClientEvent("banking:updateBalance9", source, newBalance, carteira)
          TriggerClientEvent("banking:removeBalance9", source, rounded)
          -- Extrato
