...
AddEventHandler('fuel:startFuelUpTick',function(pumpObject,ped,vehicle)
	...
	while isFueling do
		...
		local fuelToAdd = math.random(1,4) / 100.0
	...
	if pumpObject then
		TriggerServerEvent('vrp_fuel:pagamento',parseInt(currentCost),false,VehToNet(vehicle),GetVehicleFuelLevel(vehicle),currentFuel2)
	end

	currentCost = 0.0
end)

RegisterNetEvent("vrp_fuel:insuficiente")
AddEventHandler("vrp_fuel:insuficiente",function(index,fuel)
...
AddEventHandler('fuel:refuelFromPump',function(pumpObject,ped,vehicle)
	...
	while isFueling do
		...
		if pumpObject then
			...
			DrawText3Ds(vehicleCoords.x,vehicleCoords.y,vehicleCoords.z + 1,"~g~E ~w~PARA CANCELAR")
			DrawText3Ds(vehicleCoords.x,vehicleCoords.y,vehicleCoords.z +0.85,"TANQUE: ~y~"..Round(currentFuel,1).."% ~w~PREÇO: ~y~$"..parseInt(currentCost))
		end
	...
	while true do
		...
		if not isFueling and ((isNearPump and GetEntityHealth(isNearPump) > 0) or (GetSelectedPedWeapon(ped) == 883325847 and not isNearPump)) then
			if IsPedInAnyVehicle(ped) and GetPedInVehicleSeat(GetVehiclePedIsIn(ped),-1) == ped then
				...
				--DrawText3Ds(vehicleCoords.x,vehicleCoords.y,vehicleCoords.z + 0.34,"~g~E ~w~PARA ABASTECER")
			else
				local vehicle = GetPlayersLastVehicle()
				local vehicleCoords = GetEntityCoords(vehicle)
