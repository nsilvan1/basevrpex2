...
local garages = {
	[1] = { locked = false, camera = { x = -341.73, y = -130.65, z = 39.01, heading = 155.07 }, driveout = { x = -343.38, y = -134.61, z = 39.01, heading = 342.50 }, drivein = { x = -343.38, y = -134.61, z = 39.01, heading = 342.50 }, outside = { x = -343.38, y = -134.61, z = 39.01, heading = 342.50 }, inside = { x = -343.38, y = -134.61, z = 39.01, heading = 342.50 }},
	
	[2] = { locked = false, camera = { x = -323.15, y = -131.34, z = 38.97, heading = 161.86 }, driveout = { x = -324.62, y = -135.8, z = 39.01, heading = 336.03 }, drivein = { x = -324.62, y = -135.8, z = 39.01, heading = 336.03 }, outside = { x = -324.62, y = -135.8, z = 39.01, heading = 336.03 }, inside = { x = -324.62, y = -135.8, z = 39.01, heading = 336.03 }},
...
local LSCMenu = Menu.new("Mechanic","CATEGORIES",0.16,0.13,0.24,0.36,0,{255,255,255,255})
...
local function DriveInGarage()
	...
	if DoesEntityExist(veh) then
		...
		LSCMenu:addPurchase("Reparar",round(250+150*(1000-GetVehicleBodyHealth(veh))/100,0),nil)
										...
										if not LSC_Config.lock then
											if IsControlJustPressed(1,201) then
												--if emP.checkPermission() then
													...
													--TriggerEvent("Notify","negado","Somente mecânicos podem modificar veículos.",10000)
												end
											else
												drawTxt("PRESSIONE  ~g~ENTER~w~  PARA ACESSAR A MECÂNICA",4,0.5,0.93,0.50,255,255,255,180)
											end
										else
											--drawTxt("ESPAÇO JA ESTA SENDO UTILIZADO",4,0.5,0.93,0.50,255,255,255,180)
										--end
									--else
										if IsControlJustPressed(1,201) then
											--if emP.checkPermission() then
...
function CanPurchase(price,canpurchase)
	if canpurchase then
		if LSCMenu.currentmenu == "main" then
			LSCMenu:showNotification("Repaired.")
		else
			LSCMenu:showNotification("Applied Modification.")
		end
		return true
	else
		LSCMenu:showNotification("~r~Insufficient money.")
		return false
	end
end
