...
--function emP.checkPermission()
	...
	--if user_id then
		--return (vRP.hasPermission(user_id,"mecanico.permissao") or vRP.hasPermission(user_id,"admin.permissao"))
	--end
--end
	...
	if user_id then
		...
		if vehicle then
			...
			if puser_id then
				...
				vRP.setSData("custom:u"..parseInt(puser_id).."veh_"..tostring(vname),json.encode(custom))
			end
		end
	end
end)